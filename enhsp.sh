...
# Heuristics to run
heuristics=("sas" "opt" "aibr" "lm_opt" "sat-hmrp" "sat-hmrph" "sat-hmrphj" "sat-hadd")
...
for problem_file in "$problem_dir"/*.pddl; do
  ...
  java -Xmx150G -jar "$executable_location" -o "$domain_file" -f "$problem_file" > "$problem_file-.plan"
