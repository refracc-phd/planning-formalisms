...
gg_avg_search_time <- ggplot(df, aes(x = domain, y = search.time.msec, fill = search.method)) +
...
gg_search_time_type <- ggplot(df, aes(x = domain, y = search.time.msec, fill = search.method)) +
  geom_boxplot() +
  scale_y_continuous(trans="log10") +
...
gg_planning_time <- ggplot(df, aes(x = domain, y = planning.time.msec, fill = search.method)) +
  geom_boxplot() +
  scale_y_continuous(trans="log10") +
...
gg_search_vs_nodes_type <- ggplot(df, aes(x = search.time.msec, y = expanded.nodes, color = domain)) +
  geom_point() +
...
gg_avg_plan_length <- ggplot(df, aes(x = domain, y = plan.length, fill = search.method)) +
  geom_boxplot() +
...
gg_grounding <- ggplot(df, aes(x = domain, y = grounding.time, fill = search.method)) +
  geom_boxplot() +
  labs(x = "Type", y = "Grounding Time (msec)", title = "Average Time Taken to Ground Actions for Each Domain Type")
...
plots <- list(
  ...
  gg_duplicates_detected_search_method, gg_plan_length_vs_search_time, gg_duplicates_by_type_problem, gg_states_vs_search_time, gg_grounding
)
